...
def get_panel_chart_display_totals():
    """Единые доход/расход для панели и подмены гистограмм: ручная правка > сводка банка > сумма операций в прокси.
    К сводке расходов банка добавляется сумма мок‑переводов (fake_history / legacy total_out), т.к. в API банка её нет."""
    real_inc, real_exp, inc_cnt, exp_cnt = calculate_stats()
    ...
    man_inc, man_exp = _manual_operations_income_expense_month()

    def pick(mkey, bank_v, real_v):
        sync_ops = manual.get("histogram_sync_with_operations", True)
        # Жёсткий override из панели только если синхронизация с операциями выключена —
        # иначе суммы должны идти из операций (кэш + manual_operations + моки).
        if manual.get(mkey) is not None and not sync_ops:
            try:
                v = round(float(manual[mkey]), 2)
                if (
                    mkey == "expense"
                    and transfer_exp_addon
                    and manual.get("expense_add_mock_transfers", True)
                ):
...
def _fake_transfer_ops_for_panel_month(skip_ids: set) -> list:
    """Операции из fake_history за текущий месяц для списка панели (вверху списка)."""
    seen_ids = set()
    out = []
    for path in _last_transfer_json_paths():
        if not os.path.isfile(path):
...
def _manual_operations_income_expense_month() -> tuple:
    """Суммы доход/расход только по manual_operations.json за текущий месяц (не в гистограмме банка)."""
    ensure_manual_operations_fresh()
    inc = exp = 0.0
    for op_id, op in manual_operations.items():
        if op_id in hidden_operations:
...
def build_operations_api_response():
    ...
    for op_id, op in operations_cache.items():
        if is_current_month(op.get("date", "")):
            ...
            if fh and _fake_op_in_current_month(fh):
    ...
    manual_entries = [
        (oid, o) for oid, o in manual_operations.items() if is_current_month(o.get("date", ""))
    ]
    ...
    month_ops.extend(_fake_transfer_ops_for_panel_month(skip_fake))
...
def request(flow: http.HTTPFlow) -> None:
    ...
    if path == "/api/operations":
        ...
        print(f"[history] Отправлено {len(response_data.get('operations') or [])} операций в панель")
        return
